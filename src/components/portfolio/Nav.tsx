    ...
    >
      ...
      {open && (
        <div className="md:hidden border-t border-border bg-background/95 backdrop-blur-lg animate-fade-in">
          <ul className="container py-4 flex flex-col gap-1 items-center">
            {links.map((l) => (
              <li key={l.href}>
                <a
                  href={l.href}
                  onClick={() => setOpen(false)}
                  className="block py-3 text-sm tracking-[0.18em] uppercase font-light text-foreground hover:text-primary transition-smooth"
                >
                  {l.label}
                </a>
              </li>
